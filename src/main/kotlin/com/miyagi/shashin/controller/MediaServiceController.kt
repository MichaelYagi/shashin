...
class MediaServiceController(
    ...
    fun getThumbnail(model: Model, request: HttpServletRequest, @PathVariable type: String, @PathVariable metadataId: String, locale: Locale): ResponseEntity<FileSystemResource> {
        ...
        if (metadataObj.isPresent && (type == "original" || type == "225" || type == "112" || type == "centered" || type == "map" || type == "gif")) {
            ...
            val message =
                messageSource?.getMessage("main.notification.mediaservice.viewed", arrayOf("IP <a href='https://ipgeolocation.io/ip-location/$userIp' target='_blank'>$userIp</a>", metadataId), locale) + "- ${
                    sdtf.format(Date())
                }"
    ...
    fun getVideoDownload(model: Model, response: HttpServletResponse, request: HttpServletRequest, @PathVariable metadataId: String, locale: Locale): ResponseEntity<FileSystemResource>? {
        ...
        if (metadataObj.isPresent && !metadataObj.get().getType().isNullOrBlank() && metadataObj.get().getType()?.contains("video")!!) {
            ...
            sdtf.timeZone = TimeZone.getTimeZone(ZoneId.systemDefault())
            val message =
                messageSource?.getMessage("main.notification.mediaservice.downloadvideo", arrayOf("IP <a href='https://ipgeolocation.io/ip-location/$userIp' target='_blank'>$userIp</a>", metadataId), locale) +"- ${
                    sdtf.format(Date())
                }"
            notifyAdmins(userIp, message)
...
//                    }
    ...
    private fun getImageFactory(model: Model,request: HttpServletRequest, response: HttpServletResponse, metadataId: String?, attachFile: Boolean = false, locale: Locale, forcePathImage: Boolean = false, isDownload: Boolean = false): ResponseEntity<StreamingResponseBody> {
        ...
        if (metadataObj.isPresent && !metadataObj.get().getType().isNullOrBlank() && metadataObj.get().getType()?.contains("image")!!) {
            ...
            sdtf.timeZone = TimeZone.getTimeZone(ZoneId.systemDefault())
            val message =
                messageSource?.getMessage("main.notification.mediaservice.accessedimage", arrayOf("IP <a href='https://ipgeolocation.io/ip-location/$userIp' target='_blank'>$userIp</a>", metadataId), locale) + "- ${
                    sdtf.format(Date())
                }"
            notifyAdmins(userIp, message)
