...
//        response["photosWithPeopleTaggedCount"] = photosWithPeopleTaggedCount
            ...
            if (cameraCounts.count() > 0) {
                ...
                for (cameraCount in cameraCounts) {
                    cameraTotals++
                    if (cameraCount.getCamera() != null && cameraCount.getCount() != null && cameraTotals <= 15 /*cameraCount.getCount()!! > maxCameraCount!! * 0.05*/) {
            ...
            if (placenameCounts.count() > 0) {
                ...
                for (placenameCount in placenameCounts) {
                    placenameTotals++
                    if (placenameCount.getPlacename() != null && placenameCount.getCount() != null && placenameTotals <= 20 /*placenameCount.getCount()!! > maxPlacenameCount!! * 0.05*/) {
            ...
            if (keywordCounts.count() > 0) {
                ...
                for (kwCount in keywordCounts) {
                    keywordCount++
                    if (kwCount.getCount() != null && keywordCount <= 15 /*kwCount.getCount()!! > maxKwCount!! * showLimit*/) {
