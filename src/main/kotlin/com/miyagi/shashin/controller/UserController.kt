...
class UserController {
    ...
    fun getLoginUser(model: Model, session: HttpSession, @RequestParam(name="error",required=false) error: String?, @RequestParam(name="msg",required=false) message: String?, locale: Locale): String {
        ...
        } else if (model.getAttribute("authority").toString() == model.getAttribute("adminRole") || model.getAttribute("authority").toString() == model.getAttribute("superRole")) {
            return "redirect:/timeline"
        } else if (model.getAttribute("authority").toString() == model.getAttribute("userRole")) {
            return "redirect:/albums"
        } else {
    ...
    @ResponseBody
    @Secured("ROLE_SUPER","ROLE_ADMIN","ROLE_USER")
    fun toggleSlideshowOrientation(model: Model, @RequestBody requestBody: JsonNode, locale: Locale): String? {
        val userMap = mapper.convertValue(requestBody, object : TypeReference<Map<String, String>>() {})

        resp["status"] = ApiResponse.FAIL.status
        resp["msg"] = messageSource?.getMessage("main.fail", null, locale)

        if (userMap.containsKey("slideshowOrientation")) {
            val slideshowOrientation = userMap["slideshowOrientation"]?.toInt()

            val currentUserObj = model.getAttribute("currentUser") as User?
            if (currentUserObj != null) {
                currentUserObj.setSlideshowOrientation(slideshowOrientation)
                userRepository?.save(currentUserObj)
                resp["status"] = ApiResponse.SUCCESS.status
                resp["msg"] = messageSource?.getMessage("main.updated", null, locale)
            }
