...
interface SearchHistoryRepository : CrudRepository<SearchHistory?, Int?> {
    ...
    @Query("SELECT * FROM searchhistory WHERE user_id = :userId AND search_type = 2 ORDER BY created_at DESC LIMIT :limit", nativeQuery = true)
    fun findImageScraperTopNByUserIdOrderByCreatedAtDesc(@Param("userId") userId: Int, @Param("limit") limit: Int): MutableIterable<SearchHistory>?
    @Query("SELECT * FROM searchhistory WHERE user_id = :userId AND search_type = 2 ORDER BY id DESC LIMIT :limit", nativeQuery = true)
    fun findImageScraperTopNByUserIdOrderByIdDesc(@Param("userId") userId: Int, @Param("limit") limit: Int): MutableIterable<SearchHistory>?
    ...
    fun countImageScraperByUserId(userId: Int): Int
    ...
    @Query("SELECT * FROM searchhistory WHERE user_id = :userId AND LOWER(term) = LOWER(:term) AND search_type = 2 GROUP BY user_id, LOWER(term)", nativeQuery = true)
    fun findImageScraperDistinctByUserIdAndTerm(@Param("userId") userId: Int, term: String): SearchHistory?
