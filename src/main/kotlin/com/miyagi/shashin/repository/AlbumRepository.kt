...
interface AlbumRepository : CrudRepository<Album?, Int?> {
    ...
    @Query("SELECT DISTINCT a.* FROM album a LEFT JOIN albumphoto ap ON ap.album_id = a.id LEFT JOIN metadata m ON ap.metadata_id = m.id WHERE m.lat IS NOT NULL AND m.lng IS NOT NULL ORDER BY name COLLATE NOCASE ASC", nativeQuery = true)
    fun findAllWithLocationOrderByAlbumName(): MutableIterable<Album>?
    @Query("SELECT DISTINCT a.* FROM user u, album a LEFT JOIN useralbum ua ON u.id = ua.user_id AND ua.album_id = a.id LEFT JOIN albumphoto ap ON ap.album_id = a.id LEFT JOIN metadata m ON ap.metadata_id = m.id WHERE m.lat IS NOT NULL and m.lng IS NOT NULL AND u.id = :userId", nativeQuery = true)
