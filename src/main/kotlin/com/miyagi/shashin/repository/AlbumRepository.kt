...
interface AlbumRepository : CrudRepository<Album?, Int?> {
    ...
    @Query("SELECT DISTINCT u.id as userId, u.username, a.id as albumId FROM user u, album a LEFT JOIN useralbum ua ON u.id = ua.user_id AND ua.album_id = a.id WHERE u.id = :userId ORDER BY RANDOM()", nativeQuery = true)
