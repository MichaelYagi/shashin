...
interface MetadataRepository : PagingAndSortingRepository<Metadata?, String?> {
   ...
   @Query("SELECT DISTINCT m.* FROM metadata m WHERE m.id NOT IN (SELECT metadata_id FROM recognitionlabelphoto) AND m.hidden = false LIMIT 0, :matchScanLimit",nativeQuery = true)
