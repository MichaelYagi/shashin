...
interface MetadataRepository : ListCrudRepository<Metadata?, String?>, PagingAndSortingRepository<Metadata, String?> {
   ...
   @Query("SELECT DISTINCT m.id, m.type, m.lat, m.lng, m.year, m.month, m.day, '/api/v1/thumbnails/225/' || m.id as thumbnailUrlSmall, '/api/v1/thumbnails/original/' || m.id as thumbnailUrlOriginal, m.video_url as videoUrl, m.original_image_width as originalImageWidth, m.original_image_height as originalImageHeight, '/api/v1/thumbnails/map/' || m.id as mapMarkerUrl, m.place_name as placeName FROM metadata m LEFT JOIN albumphoto ap ON m.id = ap.metadata_id LEFT JOIN useralbum ua ON ap.album_id = ua.album_id LEFT JOIN album a ON a.id = ua.album_id WHERE m.hidden = 0 AND ua.user_id = :userId AND m.lat IS NOT NULL AND m.lat != \"\" AND m.lng IS NOT NULL AND m.lng != \"\" ORDER BY m.year DESC, m.month DESC, m.day DESC, m.time DESC", nativeQuery = true)
   ...
   @Query("SELECT metadata.id, metadata.type, metadata.lat, metadata.lng, metadata.year, metadata.month, metadata.day, '/api/v1/thumbnails/225/' || metadata.id as thumbnailUrlSmall, '/api/v1/thumbnails/original/' || metadata.id as thumbnailUrlOriginal, metadata.video_url as videoUrl, metadata.original_image_width as originalImageWidth, metadata.original_image_height as originalImageHeight, '/api/v1/thumbnails/map/' || metadata.id as mapMarkerUrl, metadata.place_name as placeName FROM metadata WHERE hidden = 0 AND lat IS NOT NULL AND lat != \"\" AND lng IS NOT NULL AND lng != \"\" ORDER BY year DESC, month DESC, day DESC, time DESC", nativeQuery = true)
   ...
   @Query("SELECT * FROM metadata WHERE hidden = 0 AND description IS NOT NULL AND description != '' AND (embedding IS NULL OR embedding = '') LIMIT :limit", nativeQuery = true)
   fun findAllWithDescriptionAndNoEmbedding(@Param("limit") limit: Int): List<Metadata>
   ...
   fun findMetadataByPersonByModified(@Param("recognitionLabelId") recognitionLabelId: Int, @Param("offset") offset: Int, @Param("limit") limit: Int): MutableIterable<Metadata>

   @Query("SELECT rl.id,rl.name,rl.cover_url as coverUrl, COUNT(distinct m.id || rl.id) AS tagCount, m.thumbnail_url_centered AS thumbnailUrlCentered FROM metadata m LEFT JOIN albumphoto ap on m.id = ap.metadata_id LEFT JOIN useralbum ua on ap.album_id = ua.album_id LEFT JOIN recognitionlabelphoto rlp on m.id = rlp.metadata_id LEFT JOIN recognitionlabel rl on rlp.recognition_label_id = rl.id WHERE m.hidden = 0 AND rl.name != :objectName AND (rlp.auto_tagged = 0 OR rlp.auto_tagged IS NULL) AND ua.user_id = :userId GROUP BY rl.id ORDER BY rl.name COLLATE NOCASE ASC", nativeQuery = true)
   fun findAlbumPhotoByPeople(@Param("userId") userId: Int, @Param("objectName") objectName: String): MutableIterable<MetadataPeople>

   @Query("SELECT DISTINCT m.* FROM metadata m LEFT JOIN albumphoto ap on m.id = ap.metadata_id LEFT JOIN useralbum ua on ap.album_id = ua.album_id LEFT JOIN recognitionlabelphoto rlp on m.id = rlp.metadata_id LEFT JOIN recognitionlabel rl on rlp.recognition_label_id = rl.id WHERE m.hidden = 0 AND rl.id = :recognitionLabelId AND (rlp.auto_tagged = 0 OR rlp.auto_tagged IS NULL) AND ua.user_id = :userId ORDER BY m.year DESC, m.month DESC, m.day DESC, m.time DESC, m.id DESC LIMIT :offset, :limit", nativeQuery = true)
   ...
   @Query("SELECT * FROM metadata m WHERE m.hidden = 0 AND NOT EXISTS (SELECT 1 FROM keywordphoto kp WHERE kp.metadata_id = m.id) ORDER BY m.id LIMIT 0, :matchScanLimit", nativeQuery = true)
   fun findWithoutKeywords(@Param("matchScanLimit") matchScanLimit: Int): MutableIterable<Metadata>
   @Query("SELECT * FROM metadata m WHERE m.hidden = 0 AND (NOT EXISTS (SELECT 1 FROM keywordphoto kp WHERE kp.metadata_id = m.id) OR NOT EXISTS (SELECT 1 FROM recognitionlabelphoto rlp WHERE rlp.metadata_id = m.id)) ORDER BY m.id LIMIT 0, :matchScanLimit", nativeQuery = true)
   ...
   @Query("SELECT * FROM metadata WHERE folder = :folder AND hidden = 0 ORDER BY year DESC, month DESC, day DESC, time DESC, id DESC LIMIT :offset, :limit", nativeQuery = true)
   ...
   fun countByPhotoAlbumByPerson(@Param("recognitionLabelId") recognitionLabelId: Int, @Param("userId") userId: Int): Int

   @Query("SELECT COUNT(DISTINCT m.id) FROM metadata m LEFT JOIN recognitionlabelphoto rlp on m.id = rlp.metadata_id LEFT JOIN recognitionlabel rl on rlp.recognition_label_id = rl.id WHERE m.hidden = 0 AND rl.id = :recognitionLabelId AND (rlp.auto_tagged = 0 OR rlp.auto_tagged IS NULL)", nativeQuery = true)
   fun countByMetadataByPerson(@Param("recognitionLabelId") recognitionLabelId: Int): Int
