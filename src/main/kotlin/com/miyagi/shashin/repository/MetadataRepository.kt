...
interface MetadataRepository : ListCrudRepository<Metadata?, String?>, PagingAndSortingRepository<Metadata, String?> {
   ...
   @Query("SELECT DISTINCT m.id, m.type, m.lat, m.lng, m.year, m.month, m.day, CONCAT('/api/v1/thumbnails/225/', metadata.id) as thumbnailUrlSmall, CONCAT('/api/v1/thumbnails/original/', metadata.id) as thumbnailUrlOriginal, m.video_url as videoUrl, m.original_image_width as originalImageWidth, m.original_image_height as originalImageHeight, CONCAT('/api/v1/thumbnails/map/', metadata.id) as mapMarkerUrl, m.place_name as placeName FROM metadata m LEFT JOIN albumphoto ap ON m.id = ap.metadata_id LEFT JOIN useralbum ua ON ap.album_id = ua.album_id LEFT JOIN album a ON a.id = ua.album_id WHERE m.hidden = 0 AND ua.user_id = :userId AND m.lat IS NOT NULL AND m.lat != \"\" AND m.lng IS NOT NULL AND m.lng != \"\" ORDER BY m.year DESC, m.month DESC, m.day DESC, m.time DESC", nativeQuery = true)
   ...
   @Query("SELECT * FROM metadata WHERE hidden = 0 AND ((lat IS NULL OR lat == \"\") OR (lng IS NULL OR lng == \"\")) AND CAST(strftime('%s', (year || '-' || (case when month < 10 then '0' || month else month end) || '-' || (case when day < 10 then '0' || day else day end) || ' ' || time)) as integer) BETWEEN CAST(strftime('%s', :startDate) as INTEGER) AND CAST(strftime('%s', :endDate) as INTEGER) ORDER BY CAST(strftime('%s', (year || '-' || (case when month < 10 then '0' || month else month end) || '-' || (case when day < 10 then '0' || day else day end) || ' ' || time)) as integer) DESC", nativeQuery = true)
   fun findMetadataIdBetweenTakenAtNoCoord(@Param("startDate") startDate: String, @Param("endDate") endDate: String): MutableList<Metadata>?
   ...
   @Cacheable(value = ["allAlbumMetadataWithCoordinates"], key = "{#userId}")
   @Query("SELECT DISTINCT m.id, m.type, m.lat, m.lng, m.year, m.month, m.day, CONCAT('/api/v1/thumbnails/225/', m.id) as thumbnailUrlSmall, CONCAT('/api/v1/thumbnails/original/', m.id) as thumbnailUrlOriginal, m.video_url as videoUrl, m.original_image_width as originalImageWidth, m.original_image_height as originalImageHeight, CONCAT('/api/v1/thumbnails/map/', m.id) as mapMarkerUrl, m.place_name as placeName FROM metadata m LEFT JOIN albumphoto ap ON m.id = ap.metadata_id LEFT JOIN useralbum ua ON ap.album_id = ua.album_id LEFT JOIN album a ON a.id = ua.album_id WHERE m.hidden = 0 AND ua.user_id = :userId AND m.lat IS NOT NULL AND m.lat != \"\" AND m.lng IS NOT NULL AND m.lng != \"\" LIMIT :offset, :limit", nativeQuery = true)
   ...
   @Query("SELECT metadata.id, metadata.type, metadata.lat, metadata.lng, metadata.year, metadata.month, metadata.day, CONCAT('/api/v1/thumbnails/225/', metadata.id) as thumbnailUrlSmall, CONCAT('/api/v1/thumbnails/original/', metadata.id) as thumbnailUrlOriginal, metadata.video_url as videoUrl, metadata.original_image_width as originalImageWidth, metadata.original_image_height as originalImageHeight, CONCAT('/api/v1/thumbnails/map/', metadata.id) as mapMarkerUrl, metadata.place_name as placeName FROM metadata WHERE hidden = 0 AND lat IS NOT NULL AND lat != \"\" AND lng IS NOT NULL AND lng != \"\" ORDER BY year DESC, month DESC, day DESC, time DESC", nativeQuery = true)
   ...
   fun findMetadataByPersonAndDate(@Param("startDate") startDate: String, @Param("endDate") endDate: String, @Param("recognitionConfidenceThreshold") recognitionConfidenceThreshold: String, @Param("recognitionLabelId") recognitionLabelId: Int): MutableList<Metadata>?
   ...
   @Query("SELECT DISTINCT m.* FROM metadata m LEFT JOIN albumphoto ap on m.id = ap.metadata_id LEFT JOIN useralbum ua on ap.album_id = ua.album_id LEFT JOIN recognitionlabelphoto rlp on m.id = rlp.metadata_id LEFT JOIN recognitionlabel rl on rlp.recognition_label_id = rl.id WHERE m.hidden = 0 AND m.taken_at >= :startDate AND m.taken_at <= :endDate AND rl.id = :recognitionLabelId AND rlp.confidence >= :recognitionConfidenceThreshold AND rlp.confidence < 1.0 AND ua.user_id = :userId ORDER BY m.year DESC, m.month DESC, m.day DESC, m.time DESC", nativeQuery = true)
   fun findAlbumPhotoByPersonAndDate(@Param("startDate") startDate: String, @Param("endDate") endDate: String, @Param("recognitionConfidenceThreshold") recognitionConfidenceThreshold: String, @Param("recognitionLabelId") recognitionLabelId: Int,@Param("userId") userId: Int): MutableList<Metadata>?

   @Query("SELECT DISTINCT m.* FROM metadata m INNER JOIN recognitionlabelphoto rlp ON m.id = rlp.metadata_id WHERE m.hidden = 0 AND confidence > :recognitionConfidenceThreshold AND confidence < 1.0 AND confidence != 0.0 AND rlp.recognition_label_id = :recognitionLabelId ORDER BY m.year DESC, m.month DESC, m.day DESC, m.time DESC", nativeQuery = true) //  LIMIT 0, :matchScanLimit - ,@Param("matchScanLimit") matchScanLimit: Int
   ...
   @Query("SELECT DISTINCT m.* FROM metadata m INNER JOIN recognitionlabelphoto rlp ON m.id = rlp.metadata_id WHERE m.hidden = 0 AND confidence > :recognitionConfidenceThreshold AND confidence < 1.0 AND confidence != 0.0 AND rlp.recognition_label_id = :recognitionLabelId AND m.taken_at >= :startDate AND m.taken_at <= :endDate ORDER BY m.year DESC, m.month DESC, m.day DESC, m.time DESC", nativeQuery = true) //  LIMIT 0, :matchScanLimit - ,@Param("matchScanLimit") matchScanLimit: Int
   fun findLowMatchesByPersonAndDate(@Param("startDate") startDate: String, @Param("endDate") endDate: String, @Param("recognitionLabelId") recognitionLabelId: Int,@Param("recognitionConfidenceThreshold") recognitionConfidenceThreshold: String): MutableList<Metadata>?
   ...
   @Query("SELECT SUM(foldercount) FROM (SELECT COUNT(DISTINCT m.folder) AS foldercount FROM metadata m WHERE m.hidden = 0 GROUP BY m.folder)", nativeQuery = true)
   ...
   fun findAllByFolderByDates(@Param("startDate") startDate: String, @Param("endDate") endDate: String, @Param("folder") folder: String): MutableList<Metadata>?
