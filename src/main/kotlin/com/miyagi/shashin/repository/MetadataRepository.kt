...
interface MetadataRepository : PagingAndSortingRepository<Metadata?, String?> {
   ...
   @Query("SELECT rl.*, COUNT(*) AS tagCount, m.thumbnail_url_centered AS thumbnailUrlCentered FROM metadata m INNER JOIN recognitionlabelphoto rlp ON m.id = rlp.metadata_id INNER JOIN recognitionlabel rl ON rl.id = rlp.recognition_label_id WHERE rl.name != 'object' GROUP BY rl.id", nativeQuery = true)
   ...
   @Query("SELECT rl.*, COUNT(*) AS tagCount, m.thumbnail_url_centered AS thumbnailUrlCentered FROM metadata m INNER JOIN recognitionlabelphoto rlp ON m.id = rlp.metadata_id INNER JOIN recognitionlabel rl ON rl.id = rlp.recognition_label_id INNER JOIN albumphoto ap ON ap.metadata_id = m.id WHERE rl.name != 'object' GROUP BY rl.id", nativeQuery = true)
