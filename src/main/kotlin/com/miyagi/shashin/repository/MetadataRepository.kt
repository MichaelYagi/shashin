...
interface MetadataRepository : ListCrudRepository<Metadata?, String?>, PagingAndSortingRepository<Metadata, String?> {
   ...
   @Query("SELECT id, year, month, day, type, file_name as fileName, thumbnail_small_width as thumbnailSmallWidth, thumbnail_small_height as thumbnailSmallHeight, thumbnail_url_small as thumbnailUrlSmall, thumbnail_url_extra_small as thumbnailUrlExtraSmall, thumbnail_url_centered as thumbnailUrlCentered FROM metadata WHERE type LIKE %:type% AND year = :year AND month = :month AND day = :day AND hidden = 0 ORDER BY year DESC, month DESC, day DESC, time DESC", nativeQuery = true)
   ...
   @Query("SELECT * FROM metadata WHERE type LIKE %:type% AND hidden = 0 AND id IN (SELECT id FROM metadata ORDER BY RANDOM() LIMIT 1)", nativeQuery = true)
