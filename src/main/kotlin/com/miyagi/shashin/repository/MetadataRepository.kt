...
interface MetadataRepository : ListCrudRepository<Metadata?, String?>, PagingAndSortingRepository<Metadata, String?> {
   ...
   @Query("SELECT * FROM metadata WHERE concat(year,'-',month,'-',day,' ',time) >= :startDate AND concat(year,'-',month,'-',day,' ',time) <= :endDate ORDER BY year DESC,month DESC,day DESC, time DESC", nativeQuery = true)
