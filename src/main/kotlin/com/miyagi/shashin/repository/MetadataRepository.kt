...
interface MetadataRepository : ListCrudRepository<Metadata?, String?>, PagingAndSortingRepository<Metadata, String?> {
   ...
   @Query("SELECT DISTINCT m.id, m.type, m.lat, m.lng, m.year, m.month, m.day, m.thumbnail_url_small as thumbnailUrlSmall, m.thumbnail_url_original as thumbnailUrlOriginal, m.thumbnail_path_small as thumbnailPathSmall, m.thumbnail_path_extra_small as thumbnailPathExtraSmall, m.thumbnail_url_extra_small as thumbnailUrlExtraSmall, m.thumbnail_path_centered as thumbnailPathCentered, m.thumbnail_url_centered as thumbnailUrlCentered, m.thumbnail_small_width as thumbnailSmallWidth, m.thumbnail_small_height as thumbnailSmallHeight, m.video_url as videoUrl, m.original_image_width as originalImageWidth, m.original_image_height as originalImageHeight, m.map_marker_url as mapMarkerUrl, m.place_name as placeName FROM metadata m LEFT JOIN albumphoto ap ON m.id = ap.metadata_id LEFT JOIN useralbum ua ON ap.album_id = ua.album_id LEFT JOIN album a ON a.id = ua.album_id WHERE m.hidden = 0 AND ua.user_id = :userId ORDER BY m.year DESC, m.month DESC, m.day DESC, m.time DESC", nativeQuery = true)
   fun findByAlbumMetadataByUserId(@Param("userId") userId: Int): MutableIterable<MetadataMinimal>
   ...
   @Query("SELECT metadata.id, metadata.type, metadata.lat, metadata.lng, metadata.year, metadata.month, metadata.day, metadata.thumbnail_url_small as thumbnailUrlSmall, metadata.thumbnail_url_original as thumbnailUrlOriginal, metadata.thumbnail_path_small as thumbnailPathSmall, metadata.thumbnail_path_extra_small as thumbnailPathExtraSmall, metadata.thumbnail_url_extra_small as thumbnailUrlExtraSmall, metadata.thumbnail_path_centered as thumbnailPathCentered, metadata.thumbnail_url_centered as thumbnailUrlCentered, metadata.thumbnail_small_width as thumbnailSmallWidth, metadata.thumbnail_small_height as thumbnailSmallHeight, metadata.video_url as videoUrl, metadata.original_image_width as originalImageWidth, metadata.original_image_height as originalImageHeight, metadata.map_marker_url as mapMarkerUrl, metadata.place_name as placeName FROM metadata WHERE hidden = 0 ORDER BY year DESC, month DESC, day DESC, time DESC", nativeQuery = true)
   fun findAllTimeline(): MutableIterable<MetadataMinimal>
