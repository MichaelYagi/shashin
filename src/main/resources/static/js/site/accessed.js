class Accessed {
    ...
    async init() {
        setTimeout(async () => {
            shashin.pageLoader(await this.loadNextPage.bind(this), ".appendAccessedPhotos", this.metadataList, this.activePage);
        }, 0);
    }
    ...
    async updateAccessed(nextPage,activePage,mediaTypeFilter) {
        ...
        if (false === this.eol) {
            ...
            data = await this.http.ajax("get", "/accessed/mediatype/" + mediaTypeFilter + "/page/" + nextPage);
        }

        const mediaContentList = [];
        if (data != null && data.hasOwnProperty("status") && data.hasOwnProperty("metadataList") && data.hasOwnProperty("favorites") && data.status === shashin.apiResponse.SUCCESS) {
            ...
            if (metadataList !== null && metadataList.length > 0) {
                ...
                for (let index in metadataList) {
                    ...
                    if ($("#photoThumbnailContainer"+metadata.id).length === 0) {
                        ...
                        let lastDate = metadataList.hasOwnProperty(index-1) ? dateFormat(metadataList[index-1].lastAccessedAt.replace(/-/g, "/"), "yyyy-m-d") : "";
                        if (this.lastDate !== "") {
                            lastDate = this.lastDate;
                            this.lastDate = "";
                        }
                        const currentDate = dateFormat(metadata.lastAccessedAt.replace(/-/g, "/"), "yyyy-m-d");
                        const nextDate = metadataList.hasOwnProperty(index+1) ? dateFormat(metadataList[index+1].lastAccessedAt.replace(/-/g, "/"), "yyyy-m-d") : "";
                        const displayCurrentDate = dateFormat(metadata.lastAccessedAt.replace(/-/g, "/"), "ddd, mmm d, yyyy");
                        ...
                        if ($("#"+currentDate).length === 0 && dateHeadingObj !== null) {
                            const headerAndBody = '<section class="dateSection" id="'+currentDate+'"><div class="dateHeader" style="margin-left: -16px; padding-left: 13px;margin-top: -24px; padding-top: 25px;margin-bottom: 0; padding-bottom: 15px;" id="dateHeader'+currentDate+'"><span id="select'+currentDate+'" class="bi-circle pe-2 day-select" style="font-size: 0.85rem;color: lightgray;display: none"></span><span class="text-muted">Accessed </span><strong>'+dateHeadingObj.display+'</strong>&nbsp;'+(dateHeadingObj.hasOwnProperty("placename")?dateHeadingObj.placename:'')+'</div><div id="dateBody'+currentDate+'" class="row" class="row" style="margin-left:-2px;"></div></section>';
                ...
                $(".appendAccessedPhotos").last().text("EOL").css("display","none");
                this.rendering = false;
                this.eol = true;
            }
        } else {
