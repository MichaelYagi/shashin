    ...
    static getCenteredOverlay({id, overlays, data, uuid, isMobile}) { return `
        ${($.inArray("isVideo", overlays) !== -1) ?
            ...
            <a class="mediaLink" id="mediaLink${id}" data-download-url="${encodeURI(data.metadata.videoUrl).replace(";", "%3B")}/download?v=${uuidv4()}"
                ...
                data-shoji-thumbnail-width="${isMobile ? Util.thumbnailHeight() : data.metadata.thumbnailSmallWidth}"
                data-shoji-thumbnail-height="${isMobile ? Util.thumbnailHeight() : data.metadata.thumbnailSmallHeight}"
                ...
                <img loading="lazy" draggable="false" data-smallthumb="${"/api/v1/thumbnails/"+(isMobile ? "centered" : "225")+"/"+data.metadata.id}?v=${uuid}" src="${"/api/v1/thumbnails/"+(isMobile ? "centered" : "225")+"/"+data.metadata.id}?v=${uuid}" class="photo-thumbnail-image" id="image${data.metadata.id}" width="${isMobile?Util.thumbnailHeight():data.metadata.thumbnailSmallWidth}" height="${isMobile?Util.thumbnailHeight():data.metadata.thumbnailSmallHeight}" data-aspect="${(data.metadata.thumbnailSmallWidth && data.metadata.thumbnailSmallHeight && data.metadata.thumbnailSmallHeight > 0) ? (data.metadata.thumbnailSmallWidth / data.metadata.thumbnailSmallHeight).toFixed(4) : '1.3333'}" style="background-color:lightgray;">
            </a>
            `
            :
            `
