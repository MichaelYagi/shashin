class TimelineTemplates {
    ...
    static TimelineGalleryCenterOverlay({metadata, mediaContent, uuid}) { return `
        <a class="mediaLink" 
            ...
            ${(metadata.type.indexOf("video") >= 0) ? `data-video="${Util.encodeHtml(mediaContent.video)}" data-poster="${(metadata.thumbnailUrlOriginal === null || metadata.thumbnailUrlOriginal === "") ? "/api/v1/thumbnails/225/"+metadata.id : "/api/v1/thumbnails/original/"+metadata.id}?v=${uuid}"` : `data-src="${"/api/v1/thumbnails/original/"+metadata.id}"`}
