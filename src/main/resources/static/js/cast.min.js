...
if (window.chrome && !window.chrome.cast) {
    ...
    a.src = "https://www.gstatic.com/cv/js/sender/v1/cast_sender.js?loadCastFramework=1",
        document.head.appendChild(a)
}
class Castjs {
    constructor(a={}) {
        var b = ["tab_and_origin_scoped", "origin_scoped", "page_scoped"];
        a.joinpolicy && -1 !== b.indexOf(a.joinpolicy) || (a.joinpolicy = "tab_and_origin_scoped"),
    ...
    _getBrowser() {
        return navigator.userAgent.toLowerCase().indexOf("firefox") > -1 ? "Firefox: Please enable casting, click here: https://googlechromecast.com/how-to-cast-firefox-to-tv/" : navigator.userAgent.toLowerCase().indexOf("opr/") > -1 ? "Opera: Please enable casting, click here: https://googlechromecast.com/how-to-cast-opera-browser-to-tv-using-google-chromecast/" : navigator.userAgent.toLowerCase().indexOf("iron safari") > -1 ? "Iron Safari: Please enable casting, click here: https://googlechromecast.com/how-to-cast-opera-browser-to-tv-using-google-chromecast/" : navigator.brave ? "Brave: Please enable casting, click here: https://googlechromecast.com/how-to-cast-brave-browser-to-chromecast/" : "This Browser"
    }
    _init(a=0) {
        if (!window.chrome || !window.chrome.cast || !window.chrome.cast.isAvailable)
            return a++ > 20 ? this.trigger("error", "Casting is not enabled in " + this._getBrowser()) : setTimeout(this._init.bind(this), 250, a);
    ...
    _isMediaLoadedChanged() {
        this._player.isMediaLoaded && setTimeout(()=>{
                if (this._player.mediaInfo) {
                    for (var b in this.device = cast.framework.CastContext.getInstance().getCurrentSession().getCastDevice().friendlyName || this.device,
                        ...
                        "TEXT" === this._player.mediaInfo.tracks[b].type && this.subtitles.push({
                            label: this._player.mediaInfo.tracks[b].name,
                            src: this._player.mediaInfo.tracks[b].trackContentId
                        });
                    var a = cast.framework.CastContext.getInstance().getCurrentSession().getSessionObj().media[0].activeTrackIds;
                    a && a.length && this.subtitles[a[0]] && (this.subtitles[a[0]].active = !0)
                }
            }
        )
    }
    _isConnectedChanged() {
        ...
        this.connected && (this.device = cast.framework.CastContext.getInstance().getCurrentSession().getCastDevice().friendlyName || this.device),
    ...
    _isPausedChanged() {
        this.paused = this._player.isPaused,
        this.paused && this.trigger("pause")
    }
    _playerStateChanged() {
        if (this.connected = this._player.isConnected,
            this.connected)
            switch (this.device = cast.framework.CastContext.getInstance().getCurrentSession().getCastDevice().friendlyName || this.device,
                this.state = this._player.playerState.toLowerCase(),
                this.state) {
                case "idle":
                    return this.state = "ended",
                        ...
                        this;
                case "buffering":
                    return this.time = Math.round(this._player.currentTime, 1),
                        this.duration = this._player.duration,
                        this.progress = this._controller.getSeekPosition(this.time, this.duration),
                        this.timePretty = this._controller.getFormattedTime(this.time),
                        this.durationPretty = this._controller.getFormattedTime(this.duration),
    ...
    off(a) {
        return a ? this._events[a] && (this._events[a] = []) : this._events = {},
            this
    }
    trigger(a) {
    ...
    cast(g, d={}) {
        ...
        if (cast.framework.CastContext.getInstance().getCurrentSession()) {
            ...
            if (a.metadata = new chrome.cast.media.GenericMediaMetadata,
            ...
            cast.framework.CastContext.getInstance().getCurrentSession().loadMedia(e).then(()=>(this.device = cast.framework.CastContext.getInstance().getCurrentSession().getCastDevice().friendlyName || this.device,
            this.paused && this._controller.playOrPause(),
                this), a=>this.trigger("error", a))
        } else
            cast.framework.CastContext.getInstance().requestSession().then(()=>{
                    ...
                    cast.framework.CastContext.getInstance().getCurrentSession().loadMedia(d).then(()=>(this.device = cast.framework.CastContext.getInstance().getCurrentSession().getCastDevice().friendlyName || this.device,
                    this.paused && this._controller.playOrPause(),
                        this), a=>this.trigger("error", a))
                }
                , a=>("cancel" !== a && this.trigger("error", a),
                    this))
    }
    seek(a, b) {
        return b && (a = this._controller.getSeekTime(a, this._player.duration)),
            this._player.currentTime = a,
            this._controller.seek(),
            this
    }
    volume(a) {
        return this._player.volumeLevel = a,
            this._controller.setVolumeLevel(),
            this
    }
    play() {
        return this.paused && this._controller.playOrPause(),
            this
    }
    pause() {
        return this.paused || this._controller.playOrPause(),
    ...
    mute() {
        return this.muted || this._controller.muteOrUnmute(),
            this
    }
    unmute() {
        return this.muted && this._controller.muteOrUnmute(),
    ...
    subtitle(a) {
        ...
        cast.framework.CastContext.getInstance().getCurrentSession().getSessionObj().media[0].editTracksInfo(b, ()=>{
                for (var b in this.subtitles)
    ...
    disconnect() {
        return cast.framework.CastContext.getInstance().endCurrentSession(!0),
