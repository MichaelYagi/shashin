...
CREATE TABLE `metadata` (
                            ...
                            `sharpness` VARCHAR(10) NOT NULL DEFAULT '1.0',
...
CREATE TABLE `user` (
                        ...
                        `slideshowFillScreen` BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE `duplicates` (
                                `id` INTEGER PRIMARY KEY AUTOINCREMENT,
                                `image_id_one` INTEGER NOT NULL,
                                `image_id_two` INTEGER NOT NULL,
                                `distance` INTEGER NOT NULL,
                                `created_at` DATETIME DEFAULT NULL,
                                FOREIGN KEY (image_id_one) REFERENCES metadata(id),
...
CREATE INDEX `idx_ap_metadata_id` ON albumphoto (`metadata_id`);
